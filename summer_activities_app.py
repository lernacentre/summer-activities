                ...
                if field.get('type') == 'enhanced_structured_literacy_session':
                    ...
                    for i, (activity, local_idx, q) in enumerate(current_questions):
                        ...
                        if local_idx == 0:
                            ...
                            # Auto-play multisensory after teach me (3 second delay)
                            if (st.session_state.get(teach_key, False) and 
                                not st.session_state.get(f"multi_autoplayed_{current_day}_{activity.get('activity_number')}", False) and
                                multisensory_audio_key and
                                time.time() - st.session_state.get(f"teach_time_{current_day}_{activity.get('activity_number')}", 0) > 3):
                                
                                play_audio_hidden(multisensory_audio_key)
                                st.session_state[f"multi_autoplayed_{current_day}_{activity.get('activity_number')}"] = True
                                st.rerun()
                            
                            # Show practice reminder (non-blocking)
                            if teaching_audio_key and multisensory_audio_key:
                                multisensory_script = activity.get('multisensory_audio_script', '')
                                
                                # Show expander only if teach me was clicked
                                if st.session_state.get(teach_key, False):
                                    with st.expander("🤹 Time to Practice!", expanded=not st.session_state.get(practice_key, False)):
                                        st.markdown(f"""
                                        <div class="practice-box">
                                            <h3 style="color: #ff9800; margin-bottom: 10px;">{multisensory_script}</h3>
                                            <p style="color: #666;">Do these movements now! Take your time.</p>
                                        </div>
                                        """, unsafe_allow_html=True)
                                        
                                        col1, col2 = st.columns(2)
                                        with col1:
                                            if st.button("✅ Done Practicing!", key=f"done_{current_day}_{activity.get('activity_number')}", use_container_width=True, type="primary"):
                                                st.session_state[practice_key] = True
                                                st.balloons()
                                                time.sleep(1)
                                                st.rerun()
                                        with col2:
                                            if st.button("🔄 Play Audio Again", key=f"replay_{current_day}_{activity.get('activity_number')}", use_container_width=True):
                                                if multisensory_audio_key:
                                                    play_audio_hidden(multisensory_audio_key)
